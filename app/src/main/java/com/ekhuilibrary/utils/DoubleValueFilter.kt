...
class DoubleValueFilter : DigitsKeyListener(false, true) {
    private var digits = 2
    fun setDigits(d: Int): DoubleValueFilter {
        digits = d
        return this
    }
...
//如果大于最大整型 无法输入
        val value = dest.toString().toDoubleOrNull()
        if (value != null && value > Int.MAX_VALUE / 10) {
            return ""
        }
