package com.example.ekhuibaselibrary.custom.treelist;
...
public abstract class TreeRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        // 设置内边距
        holder.itemView.setPadding(node.getLevel() * 50, 0, 10, 0);
        /**
    ...
    public String getSelectedNodeID() {
        ...
        if (checks.length() == 0)
            return "";
        return checks.toString().substring(0, checks.toString().length() - 1);
    }
    /**
     * 获取所有选中节点的Name
     *
    ...
    public String getSelectedNodeName() {
        ...
        for (int i = 0; i < mAllNodes.size(); i++) {
            Node n = mAllNodes.get(i);
            if (n.isChecked()) {
                checks.append(n.getName()).append(",");
            }
        }
        if (checks.length() == 0)
            return "";
